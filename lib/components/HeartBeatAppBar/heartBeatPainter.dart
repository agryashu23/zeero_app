...
class HeartBeatPainter extends CustomPainter {
  ...
  double parabola(double x) => pow(((cos(x) + 1) / 2), 10).toDouble();
  ...
  HeartBeatPainter({
    ...
    if (!inversed) {
      ...
      heartBeatPaint.shader = LinearGradient(
        colors: <Color>[Colors.pink.shade300, Colors.white],
        stops: [0.0, 0.3],
      ).createShader(
      ...
      heartBeatPaint.shader = LinearGradient(
        colors: <Color>[Colors.white, Colors.pink.shade300],
        stops: [0.7, 1.0],
      ).createShader(
        Rect.fromPoints(_points[0], _points[_points.length - 1]),
      );
  ...
  void paint(Canvas canvas, Size size) {
    double percentage = _points.length.toDouble() * animation?.value;

    int start = lerp(0, percentage, pow(sigmoid(pow(animation?.value, 5).toDouble()), 1).toDouble()).floor();
